...
Inspector.prototype.inspectObject = function (object) {
    ...
    if (!inspectedObjects || !this.multipleSelection) {
        ...
        for (var name in declaredProperties) {
            ...
            let title = "title" in desc ? desc["title"] : name;
            tbody.appendChild(this.buildPropertyRow(name,title, val));
        }
        ...
        if (this.includesOwnProperties) {
            for (var k in data) {
                ...
                tbody.appendChild(this.buildPropertyRow(k, k, data[k]));
            }
        }
        ...
        while (mapIt.next()) {
            tbody.appendChild(this.buildPropertyRow(mapIt.key, mapIt.key, mapIt.value)); // shows the properties that are allowed
        }
        table.appendChild(tbody);
        mainDiv.appendChild(table);
 ...
 */
Inspector.prototype.buildPropertyRow = function (propertyName, title, propertyValue) {
    var mainDiv = this._div;
    ...
    if (decProp && decProp.type === "select") {
        ...
        if (decProp && decProp.type === "textarea") {
